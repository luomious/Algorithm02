...
//        System.out.println(Arrays.toString(edges));
    ...
    public KruskalCase(char[] vertexs, int[][] matrix) {
        ...
        for (int i = 0; i < vlen; i++) {
            for (int j = i + 1; j < vlen; j++) {
                if (this.matrix[i][j] != INF) {//表示两个点连通
                    edgeNum++;
                }
            }
        }
    ...
    //对边进行排序
    private void sortEdges(EData[] edges) {//edges边的集合
    ...
    //获取图中的边,放到EData[]数组中,遍历该数组,通过邻接矩阵来获取,如['A','B',12]
    ...
    private int getEnd(int[] ends, int i) {//ends[]是动态变化的
        while (ends[i] != 0) {
            i = ends[i];
        }
        return i;
